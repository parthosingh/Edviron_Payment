...
export class EdvironPgController {
  ...
  async handleCallback(@Req() req: any, @Res() res: any) {
    try {
      ...
      if (info.gateway !== Gateway.PENDING && info.gateway !== Gateway.EDVIRON_PG) {
  ...
  async handleEasebuzzCallback(@Req() req: any, @Res() res: any) {
    ...
    if (collectRequest.gateway !== Gateway.PENDING && collectRequest.gateway !== Gateway.EDVIRON_EASEBUZZ) {
  ...
  async webhookTrigger(
    ...
    try {
      ...
      let noUrlCount = 0;
      await Promise.all(
        aggregateData.map(async (item: any) => {
          if (item?.req_webhook_urls && item?.req_webhook_urls?.length > 0) {
            for (const url of item.req_webhook_urls) {
              try {
                ...
                try {
                  ...
                  console.log(
                    response.data,
                    'response from webhook for collect_id:',
                    item.collect_id,
                    'url:',
                    url,
                  );
                  ...
                  failCount++;
                  console.log(
                    'Error in webhook for collect_id:',
                    item.collect_id,
                    'url:',
                    url,
                    e.message,
            ...
            console.log(
              'No webhook url found for collect_id:',
              item.collect_id,
            );
            noUrlCount++;
          }
  ...
  async rzpOrderDetail(@Query('order_id') order_id: string): Promise<{
    razorpay_seamless: I_Razorpay;
  }> {
    try {
      const collect_request =
