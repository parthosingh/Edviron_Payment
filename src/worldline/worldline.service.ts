...
export class WorldlineService {
    ...
    async SingleUrlIntegeration(request: CollectRequest): Promise<{ url: string; collect_req: CollectRequest }> {
        const {
            _id,
            amount,
            additional_data,
        ...
        if (!vendorDetails.length) {
            ...
            items = vendorDetails.map((vendor: {
                vendor_id: string;
                percentage?: number;
                amount?: number;
                name?: string;
        ...
        const encryptedData =
            await this.encryptTxthdnMsg(JSON.stringify(plainJson), worldline_encryption_key, worldline_encryption_iV);
        ...
        try {
            ...
            const decryptedData =
                await this.decryptAES256Hex(recieveHexData, worldline_encryption_key, worldline_encryption_iV);
    ...
    async encryptTxthdnMsg(plainText: string, encryptionKey: string, iv: string): Promise<string> {
    ...
    async decryptAES256Hex(hexData: string, encryptionKey: string, iv: string): Promise<string> {
    ...
    async getStatus(collect_id: string): Promise<any> {
        ...
        const encryptedData =
            await this.encryptTxthdnMsg(
                JSON.stringify(plainJson), worldline_encryption_key, worldline_encryption_iV
            );
        ...
        try {
            ...
            const decryptedData = await this.decryptAES256Hex(
                recieveHexData, worldline_encryption_key, worldline_encryption_iV
            );
            const parsedData = JSON.parse(decryptedData);
            // return parsedData;
    ...
    async formatWorldlinePaymentStatusResponse(
        ...
        try {
            ...
            if (response?.paymentMethod?.bankSelectionCode === '470') {     //netbanking
